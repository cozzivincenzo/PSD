...
int scontaLibri(Libro *biblioteca, int anno, float percentuale, int n){
    ...
    FILE *output_file=fopen("output.txt", "w"); // Dichiarazione e apertura in scrittura del file atto a contenere la biblioteca con i dati aggiornati
    ...
    /* Scrittura sul file i dati aggiornati della biblioteca */

    for(int i=0; i<n; i++){
        fprintf(output_file, "%s\n%s\n%d\n%0.2f\n", getTitolo(biblioteca[i]), getEditore(biblioteca[i]), getAnno(biblioteca[i]), getPrezzo(biblioteca[i]));
    }

    fclose(output_file); // Chiusura del file
    return flag;
...
int eliminaLibri(Libro *biblioteca, int anno, int n){
    ...
    /* Ciclo utile alla riscrittura della vecchia biblioteca sulla base della nuova, per poi scriverla sul file */
    ...
    fclose(output_file); // Chiusura del file 
    liberaBiblioteca(tmp); // Liberazione dell'allocazione di memoria
...
void trovaLibriPrezzoSimile(Libro* biblioteca, int n){
    ...
    printf("\nI libri con scarto di prezzo minore sono '%s' e '%s', con uno scarto di %0.2f", getTitolo(output1), getTitolo(output2), min);

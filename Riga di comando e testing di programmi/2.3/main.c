...
int run_test_case(char *tc_id, int n, int val, int pos){
    ...
    int input_array[n+1]; // Dichiarazione degli array atti a contenere i dati dei file di input
    ...
    inserimento(input_array, pos, val, n); // Chiamata alla funzione per l'inserimento dei valori nelle posizioni desiderate
...
{ 	
    ...
    int n, pos, val, pass; // Dichiarazione di variabili utili ai controlli
    ...
    while(fscanf(test_suite, "%s%d%d%d", tc_id, &n, &val, &pos) == 4){

        pass = run_test_case(tc_id, n, val, pos); // Chiamata alla funzione principale

...
/* FUNZIONE UTILE ALLA CREAZIONE DI UNA SOTTOLISTA */
list sottolista(list lista, list sublist, int sinistro, int destro, int i) {

     /* Se l'indice supera il limite sinistro, la funzione si stoppa */
     if(i<sinistro){
         return sublist;
     }

     /* Se l'indice è tra i limiti, viene riempita la sottolista */
     if(i>=sinistro&&i<=destro){
          sublist=consList(getFirst(lista), sublist);
     }
     lista=tailList(lista); // Scorrimento della lista 

     sublist=sottolista(lista, sublist, sinistro, destro, i-1); // Richiamo della funzione diminuendo l'indice
